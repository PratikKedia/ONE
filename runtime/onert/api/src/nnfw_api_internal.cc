...
void nnfw_session::make_dependency()
{
  for (uint32_t out_exe = 0; out_exe < _executions.size(); out_exe++)
  {
    ...
    {
      if (out_exe == in_exe) continue;
      auto in_graph = _executions[in_exe]->primary_subgraph();
      for (auto out = out_graph._name_to_output_begin(); out != out_graph._name_to_output_end(); out++) {
        ...
        for (auto in = in_graph._name_to_input_begin(); in != in_graph._name_to_input_end(); in++)
        {
          ...
          if (is_same) _executions[out_exe]->pushNextExe(_executions[in_exe], out->second, in->second);
        }
      }
    }
  }
...
NNFW_STATUS nnfw_session::push_pipeline_input(std::vector<void *> *inputs, std::vector<uint32_t> *lengths)
{
  ...
  _executions[0]->createNewAsyncDesc(count++); // to be fixed
...
NNFW_STATUS nnfw_session::pop_pipeline_output(std::vector<void *> *outputs)
{
  ...
  while (results->empty())
  {
    if (_executions[_executions.size() - 1]->stopWait()) return NNFW_STATUS_ERROR;
  }

  auto result = results->front();
  results->pop_front();
  for (uint32_t i = 0; i < result.size(); i++) outputs->push_back(result[i]);
  return NNFW_STATUS_NO_ERROR;
}
