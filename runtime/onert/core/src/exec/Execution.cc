  ...
  while (true)
  {
    ...
    else
    {
      ...
      for (uint32_t i = 0; i < output_sz; i++)
      {
        ...
        auto dtype = datatype_to_nnfw_dtype(primary_subgraph().operands().at(opidx).typeInfo().type());
        ...
        executeAsyncOutput(onert::ir::IOIndex(i), buffer, tensor_size);
      }
      AsyncExecute();
      inference_cnt += 1;
