...
void Execution::createNewAsyncDesc(uint32_t count)
{
  ...
  _async_io_desc->inputs_sem.resize(primary_subgraph().getInputs().size());
  for (uint32_t i = 0; i < _async_io_desc->inputs_sem.size(); i++)
  {
    _async_io_desc->inputs_sem.at(i) = new sem_t;
    sem_init(_async_io_desc->inputs_sem.at(i), 0, 0);
  }
                                  ...
                                  ir::Layout layout)
{
  ...
  sem_post(_async_io_desc->inputs_sem.at(index.value()));
}
...
void Execution::ioDescSemWait()
{
  auto semaphores = _async_io_descs.front().first->inputs_sem;
...
void Execution::asyncIoDescSemWait()
{
  sem_wait(&_async_io_descs_sem);
}

void Execution::asyncIoDescSemPost()
{
  sem_post(&_async_io_descs_sem);
}
  ...
  while (true)
  {
    ...
    else
    {
      ...
      for (uint32_t i = 0; i < output_sz; i++)
      {
        ...
        if (dtype == onert::ir::DataType::FLOAT32 || dtype == onert::ir::DataType::INT32 || dtype == onert::ir::DataType::UINT32) tensor_size *= 4;
        else if (dtype == onert::ir::DataType::INT64) tensor_size *= 8;
      ...
      for (uint32_t i = 0; i < next_exes.size(); i++)
      {
        ...
        for (auto iter = next_io_descs->begin(); iter != next_io_descs->end(); iter++)
        {
          if (inference_cnt == iter->second)
          {
            ...
            iter->first->inputs.at(i_index.value()) = std::make_unique<onert::exec::InputDesc>(info, _buffer, length, onert::ir::Layout::NHWC);
            sem_post(iter->first->inputs_sem.at(i_index.value()));
            break;
          }
        }
      ...
      for (uint32_t i = 0; i < _io_desc->inputs_sem.size(); i++) {
        auto p = _io_desc->inputs_sem.at(i);
        if (p) delete p;
      }
      for (uint32_t i = 0; i < _io_desc->outputs.size(); i++) {
